...
5. **Archive does not cascade**: archiving a system sets only the system's `archivedAt`. Member items stay active and **keep their `systemId` link** to the archived system (the relation is preserved so the item still shows "Part of {archived} HVAC" rather than orphaning). The user archives components individually if they want to. Hard-deleting a system, by contrast, uses `onDelete: SetNull` on `Item.systemId` and does clear the link — that path is for when a system was never actually installed or is being removed entirely.
